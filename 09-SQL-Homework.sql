SELECT * FROM actor;
-- Display all actors' first and last name
SELECT first_name, last_name FROM actor;

-- Combine actor first and last name into one column
SELECT CONCAT_WS('', first_name, ' ', last_name) AS 'Actor Name' 
  FROM actor;

-- Find actor ID, first name, and last name of actors named "Joe"  
...
UPDATE actor
SET first_name = CASE WHEN first_name = 'HARPO' THEN first_name = 'GROUCHO'
END;

SELECT CASE WHEN first_name != 'GROUCHO' THEN 'JOE' ELSE FIRST_NAME END,
FIRST_NAME,
LAST_NAME
FROM SAKILA.ACTOR;


-- The above code deleted all the first names of the actor table!
...
SELECT SUM(payment.amount), staff.staff_id, staff.last_name, staff.first_name
FROM staff
...
SELECT COUNT(film_actor.actor_id), film_actor.film_id, film.film_id
FROM film
...
SELECT COUNT(inventory.film_id), film.film_id, film.title
FROM film
...
GROUP BY customer.customer_id ORDER BY customer.last_name, customer.first_name ASC;
...
FROM(
	SELECT address_id 
    FROM(
		SELECT country, city.country_id, city.city_id
        FROM country as C 
        INNER JOIN city as city ON
        C.country_id=city.country_id WHERE country='Canada') AS country_city
		INNER JOIN address as address ON
        country_city.city_id=address.city_id WHERE 1) AS with_address
        INNER JOIN customer ON with_address.address_id=customer.address_id;
